    ...
    useEffect(() => {
        ...
        for (const [day, val] of Object.entries(selected)) {
            for (const [meal, mealV] of Object.entries(val)) {
                if (mealV) {
                    const thisCost = Number(menu[dayToNum[day]][meal].cost || 0); // ✅ Ensure cost is number
                    total += thisCost;
                }
            }
        }
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                let data = {};
                for (let c of cost.data) {
                    data[c.meal] = Number(c.cost); // ✅ convert cost to number here
                }
